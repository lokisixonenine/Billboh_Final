  ...
  return (
    ...
    <Layout style={{background: 'url(https://www.hobbitontours.com/media/1174/hobbiton-movie-set-30.jpg) bottom / cover'}}>
        <Header transparent title="Billboh" style={{color: 'white'}}>
            <Navigation>
                <a href="billboh/client/src/components/cityHistory.js">City History</a>
...
class MyComponent extends React.Component {
  ...
  componentDidMount() {
    fetch("https://en.wikipedia.org/w/api.php?action=query&origin=*&format=json&generator=search&gsrnamespace=0&gsrlimit=35&gsrsearch=")
      .then(res => res.json())
      .then(
        (result) => {
          this.setState({
